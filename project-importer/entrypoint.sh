...
for project_name in ${project_names}; do
  for downstream_project_name in $(downstream_project_names $project_name); do
    ...
    trigger_token="$(create_project_pipeline_trigger ${private_token} ${downstream_project_id} ${project_name} | jq --raw-output '.token')"
    key="$(echo "${downstream_project_name}_TRIGGER_TOKEN" | tr '-' '_' | tr '[:lower:]' '[:upper:]')"
    create_project_level_variable ${private_token} ${project_id} ${key} ${trigger_token}
  done
done
...
initial_trigger="$(create_project_pipeline_trigger ${private_token} ${initial_project_id} 'Initial project trigger')"
initial_trigger_token="$(echo ${initial_trigger} | jq --raw-output '.token')"

trigger_project_pipeline ${private_token} ${initial_project_id} ${initial_trigger_token} 'master'
